/**********************************************************************************************
***********************************************************************************************
Macro Name: MULTIPLE_PHREGFirth V1. It is derived from MULTIPLE_PHREG V21
...
updated date/ Person: 04/04/2018  Chao Zhang
...
%macro MULTIPLE_PHREG_Firth(OUTPATH=, FNAME =, FOOTNOTE=, FOOTNOTE2=, TYPE3=T, EFFECT=,SLICEBY=,CLNUM=T,STRATA=, shortreport=T,
  ...
  %if &effect ~=%str() %then %do;
   ...
   DATA mergedata;
      ...
      if HazardRatio ~= . then HR = TRIM(LEFT(PUT(HazardRatio,8.2))) || " (" || 
         TRIM(LEFT(PUT(HRLowerPLCL,8.2))) || "-" || 
         TRIM(LEFT(PUT(HRUpperPLCL,8.2))) || ")";
      else HR = '-';
